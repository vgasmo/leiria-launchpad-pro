  ...
  try {
    ...
    let emailHtml = `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        <h1 style="color: #1a1a1a; font-size: 24px; margin-bottom: 8px;">Session Follow-up: ${session.title}</h1>
        <p style="color: #666; font-size: 14px; margin-bottom: 24px;">${startupName} • ${sessionDate}</p>
        
        ${session.ai_summary ? `
          <div style="background: #f8f9fa; padding: 16px; border-radius: 8px; margin-bottom: 24px;">
            <h2 style="color: #1a1a1a; font-size: 16px; margin: 0 0 12px 0;">📋 Summary</h2>
            <p style="color: #333; font-size: 14px; line-height: 1.6; margin: 0; white-space: pre-wrap;">${session.ai_summary}</p>
          </div>
        ` : ""}
        ...
        ${decisions.length > 0 ? `
          <div style="margin-bottom: 24px;">
            ...
            <ul style="margin: 0; padding-left: 20px;">
              ${decisions.map(d => `<li style="color: #333; font-size: 14px; margin-bottom: 8px;">${d}</li>`).join("")}
            </ul>
          </div>
        ` : ""}
              ...
              ${actionSuggestions.map(action => `
                <tr style="border-bottom: 1px solid #eee;">
                  <td style="padding: 12px 0;">
                    <strong style="color: #1a1a1a; font-size: 14px;">${action.title}</strong>
                    ${action.description ? `<p style="color: #666; font-size: 13px; margin: 4px 0 0 0;">${action.description}</p>` : ""}
                  </td>
                  <td style="padding: 12px 0; text-align: right; vertical-align: top;">
                    <span style="background: ${action.priority === 'urgent' ? '#fee2e2' : action.priority === 'high' ? '#fef3c7' : '#e0f2fe'}; color: ${action.priority === 'urgent' ? '#dc2626' : action.priority === 'high' ? '#d97706' : '#0284c7'}; padding: 4px 8px; border-radius: 4px; font-size: 12px; text-transform: uppercase;">${action.priority}</span>
                  </td>
                </tr>
              `).join("")}
            </table>
          ...
          <div style="background: #fffbeb; padding: 16px; border-radius: 8px; margin-bottom: 24px;">
            ...
            ${kpiPrompts.map(kpi => `
              <div style="margin-bottom: 12px;">
                <strong style="color: #1a1a1a; font-size: 14px;">${kpi.kpiName}</strong>
                <p style="color: #666; font-size: 13px; margin: 4px 0 0 0;">${kpi.reason}</p>
              </div>
            `).join("")}
          </div>
        ` : ""}
        ...
        <div style="margin-top: 32px; padding-top: 16px; border-top: 1px solid #eee;">
          <p style="color: #999; font-size: 12px; margin: 0;">
            Sent by ${senderProfile?.full_name || "Startup Leiria"} via Startup Leiria Platform
          </p>
        </div>
      </div>
    `;
    ...
    const subject = `Follow-up: ${session.title} - ${startupName}`;
