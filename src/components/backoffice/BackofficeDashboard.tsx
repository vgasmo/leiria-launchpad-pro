          ...
          <CardContent className="pt-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground">{t('backoffice.dashboard.activeContracts', 'Active Contracts')}</p>
              </div>
          ...
          <CardContent className="pt-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground">{t('backoffice.dashboard.monthlyRevenue', 'Monthly Revenue')}</p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {t('backoffice.dashboard.occupancy', 'Occupancy')} ({data.occupiedRooms}/{data.totalRooms})
                </p>
              </div>
          ...
          <CardContent className="pt-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {t('backoffice.dashboard.waitingList', 'Waiting List')}
                  {data.highPriorityWaiting > 0 && (
                    <span className="text-orange-600 ml-1">({data.highPriorityWaiting} {t('backoffice.dashboard.urgent', 'urgent')})</span>
                  )}
                </p>
              </div>
        ...
        <Card className={cn(data.criticalAlerts > 0 && 'border-red-500/50')}>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              {t('backoffice.dashboard.alerts', 'Alerts & Anniversaries')}
              {data.criticalAlerts > 0 && (
                <Badge variant="destructive" className="ml-2">{data.criticalAlerts} {t('backoffice.dashboard.critical', 'critical')}</Badge>
              )}
            </CardTitle>
            <CardDescription>
              {t('backoffice.dashboard.alertsDesc', 'Upcoming anniversaries, contract renewals, and 3-year milestones')}
            </CardDescription>
          </CardHeader>
          <CardContent>
            {data.alerts.length === 0 ? (
              <div className="flex items-center gap-2 text-muted-foreground py-4">
                ...
                {t('backoffice.dashboard.noAlerts', 'No upcoming alerts')}
              </div>
            ) : (
                            ...
                            <>
                              <span className="font-medium text-red-600">
                                {alert.yearsIncubated >= 3 
                                  ? t('backoffice.dashboard.over3Years', '3+ years incubated')
                                  : t('backoffice.dashboard.approaching3Years', 'Approaching 3-year mark')}
                              </span>
                              {' — '}
                              {t('backoffice.dashboard.priceIncreaseOrExit', 'Review for price increase or graduation')}
                            </>
                          )}
                          {alert.alertType === 'anniversary' && (
                            <>
                              {t('backoffice.dashboard.yearAnniversary', 'Year {{year}} anniversary', { year: alert.yearsIncubated })}
                              {' — '}
                              {alert.daysUntilAnniversary} {t('backoffice.dashboard.daysAway', 'days away')}
                            </>
                          )}
                          {alert.alertType === 'expiring' && (
                            <>
                              {t('backoffice.dashboard.contractExpiring', 'Contract expiring')}
                              {' — '}
                              {alert.daysUntilAnniversary} {t('backoffice.dashboard.daysRemaining', 'days remaining')}
                            </>
                          )}
                        </div>
                        <div className="text-xs text-muted-foreground mt-1">
                          {t('backoffice.dashboard.incubatedSince', 'Incubated since')} {format(new Date(alert.startDate), 'MMM yyyy')} 
                          {' '}({alert.monthsIncubated} {t('backoffice.dashboard.months', 'months')})
                        </div>
                      </div>
                      <Button 
                        variant="ghost" 
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              {t('backoffice.dashboard.tenureBreakdown', 'Incubation Tenure')}
            </CardTitle>
            <CardDescription>
              {t('backoffice.dashboard.tenureDesc', 'Distribution of startups by time incubated')}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {[
                { key: 'under1year', label: t('backoffice.dashboard.under1Year', '< 1 year'), color: 'bg-green-500' },
                { key: '1to2years', label: t('backoffice.dashboard.1to2Years', '1-2 years'), color: 'bg-blue-500' },
                { key: '2to3years', label: t('backoffice.dashboard.2to3Years', '2-3 years'), color: 'bg-yellow-500' },
                { key: 'over3years', label: t('backoffice.dashboard.over3Years', '3+ years'), color: 'bg-red-500' },
              ].map(item => {
